package com.example.myslidingmenu2.view;
...
public class MySlidingMenuView extends ViewGroup {
	...
	/**
	 * 此属性控制BODY是否可以滑动出界,
	 * 通过setlSlidingOutBorder(boolean lSlidingOutBorder)方法设置
	 */
	...
	public static boolean menuclose = true; //没打开menu
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		System.out.println("-------------"+child1.getLayoutParams().width+"/"+child1.getLeft()+"/"+child1.getRight());
		final View child2 = getChildAt(1);
		child2.measure(widthMeasureSpec, heightMeasureSpec);
		System.out.println("child1.getLayoutParams().width="+widthMeasureSpec);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if ((action == MotionEvent.ACTION_MOVE) && (lTouchState != TOUCH_STATE_REST)) {
			return true;
		}
		...
		case MotionEvent.ACTION_DOWN:
			lTouchX = x;
			lTouchY = y;
			lTouchState = lScroller.isFinished() ? TOUCH_STATE_REST : TOUCH_STATE_SCROLLING;
			break;
...
//			if (lTouchState == TOUCH_STATE_SCROLLING) {
			...
			isSlidingState = true;
			lScroller.startScroll(getScrollX(), 0, lTouchUpOffux - getScrollX(), 0, DURATION_TIME);
			menuclose = true;
			invalidate();
...
//			if (lTouchState == TOUCH_STATE_SCROLLING) {
			...
			int offx = 0;// 此次移动的偏移量
			// 计算拉动是否超出边缘
			if ((getScrollX() + (int) (lTouchX - event.getX()) < 0) || (getScrollX() + (int) (lTouchX - event.getX()) > LEFT_VIEW_WIDTH)) {// VIEW已经被拉过边缘
...
//			}
	...
	public void closeMenu(){
		while(!menuclose) {
		lScroller.startScroll(getScrollX(), 0, LEFT_VIEW_WIDTH, 0, DURATION_TIME);
		menuclose = true;
		lTouchState = TOUCH_STATE_REST;
		invalidate();
		}
	...
	/**
	 * 设置BODY是否可以滑动出边界范围
	 * @param lSlidingOutBorder true可以滑动出边界范围
	 */
